				 ...
				 int*	curIndex)
{
	...
	if (l_curIndex != 0)
	{
	  ...
	  f   l_interOffsetSizes1.z = (l_interOffsetSizes2.z + l_interOffsetSizes2.w);
	}
							...
							int	taskNum)
{
	...
	for (int i = recordBase + tid; i < terminate; i+=blockDim.x)0
	{
		int cindex = i;
...
//printf("g_spec->outputAllKeySize=%d\n",g_spec->outputAllKeySize);
